...
python3 -m unittest bin/tests/test_nutrition -v

# Run a single test method
python3 -m unittest bin.tests.test_nutrition.TestNutrition.test_method_name -v
```
...
1. **Skills** (top-level directories: `snap/`, `health/`, `insights/`, `news/`, `daily-coach/`, `health-qa/`) — Agent-facing markdown prompts that guide when/how to use each tool. Registered in `openclaw.plugin.json`.
...
3. **Scripts** (`bin/`) — Deterministic Python helpers that do the actual work. Each subdirectory maps to a tool domain: `nutrition/`, `health/`, `insights/`, `news/`, `coach/`, `common/`.
...
All runtime data lives in `longevityOS-data/` (gitignored). Subdirectories: `nutrition/`, `health/`, `insights/`, `news/`. Seed data in `bin/seed/` can bootstrap a fresh install. Python scripts accept `--data-root` to locate this directory.

### Tool-to-script mapping

| Tool | Python script(s) |
|------|------------------|
| `nutrition` | `bin/nutrition/estimate_and_log.py`, `daily_summary.py`, `weekly_summary.py` |
| `health_profile` | `bin/health/profile_store.py` |
| `whoop_initiate` | `bin/health/import_whoop.py` |
| `whoop_sync` | `bin/health/import_whoop.py` + `profile_store.py` (chained) |
| `experiments` | `bin/insights/experiments.py` |
| `news_digest` | `bin/news/fetch_digest.py` |
| `coaching_context` | `bin/coach/daily_health_coach.py` |
| `learnings` | `bin/common/learnings.py` |
...
Tests are in `bin/tests/` using Python's `unittest`. Whoop API fixtures live in `bin/tests/fixtures/whoop/`. Tests use sanitized real API responses. Each test file maps to a script domain (e.g., `test_nutrition.py` tests `bin/nutrition/`).

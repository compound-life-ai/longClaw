...
大多数健康 App 都是**记录器**。它们只是把你告诉它的数据记下来，再画几张图表。它们不会思考，不会把你的睡眠、饮食和化验结果串在一起分析，也不会去翻论文弄清楚你的 CRP 为什么在升高。

Turri 不一样。它是一个 **agentic system**：由 10 位专科 AI 医师组成的团队，主动分析你的健康数据。

| 现有方案 | Turri 的做法 |
|---|---|
| 手动记录饮食 | 通过照片或文字记录饮食，基于确定性目录 + USDA API 估算营养成分，并学习你的常用食谱 |
| 看一张体重曲线 | 发现你在蛋白质 >150g/天 且 睡眠 >7h 的那几周体重下降更快 |
| 用表格追踪补剂 | 标记药物交互（NIH ODS），追踪依从性，与进行中的试验对齐 |
| 从 AI 聊天中获取泛泛的健康建议 | 在 PubMed 和 bioRxiv 上搜索与你个人数据模式匹配的证据 |
| 看完体检报告就忘了 | 构建持久化的健康画像，让每一条建议都随时间越来越精准 |
| 好奇 Instagram 上看到的东西到底有没有用 | 设计对照自我实验、追踪执行、做因果分析——数据不够时直接说「数据不足」 |
...
<table>
<tr>
<td align="center" width="20%"><img src="bin/docs/characters/yuyi.svg" alt="太医" width="80"/><br/><b>太医</b><br/><sub>总协调者，综合得出今日第一优先级</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/shiyi.svg" alt="食医" width="80"/><br/><b>食医</b><br/><sub>营养：宏量、微量、食物建议</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/daoyin.svg" alt="导引师" width="80"/><br/><b>导引师</b><br/><sub>运动：按恢复状态调整训练</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/zhenmai.svg" alt="脉诊师" width="80"/><br/><b>脉诊师</b><br/><sub>体征：RHR、HRV、SpO₂ 趋势</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/yanfang.svg" alt="验方师" width="80"/><br/><b>验方师</b><br/><sub>生物标志物：跨领域模式识别</sub></td>
</tr>
<tr>
<td align="center" width="20%"><img src="bin/docs/characters/bencao.svg" alt="本草师" width="80"/><br/><b>本草师</b><br/><sub>补剂：微量营养素缺口分析</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/shixiao.svg" alt="试效官" width="80"/><br/><b>试效官</b><br/><sub>实验：执行情况追踪</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/yuanpan.svg" alt="院判" width="80"/><br/><b>院判</b><br/><sub>试验设计：N-of-1 候选方案</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/yizheng.svg" alt="医正" width="80"/><br/><b>医正</b><br/><sub>安全审核：过度训练与异常下降信号</sub></td>
<td align="center" width="20%"><img src="bin/docs/characters/baogao.svg" alt="报告官" width="80"/><br/><b>报告官</b><br/><sub>报告：相关研究与文献</sub></td>
</tr>
</table>
...
每个工具都包了一层 `bin/` 里的 Python 脚本。SDK 入口（`index.ts`）通过 `execFile` 去调用这些脚本。

顶层目录（`snap/`、`health/` 等）里的 skill 为 agent 提供使用指引，例如什么时候调用哪个工具、如何组织结果。工具本身则提供 OpenClaw 可注册、可检查的类型化接口。
...
测试使用了来自 `bin/tests/fixtures/whoop/` 的真实（已脱敏）Whoop API 响应样本。
